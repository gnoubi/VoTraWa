...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	float qt_female_to_eat <- 30 * 80#gram/(#km*#km)/2;
	//float vole_eat_qte <-30;
	//float vole_eat_surfacique_mass <- vole_eat_qte*80#gram/(100#m*100#m);
	float adult_eat_proportion <- 0.5;
	float juvenile_eat_proportion <- 0.5;
...
//	float population;
	...
	reflex discover when: (!empty(used_knowledge) )
	{
		...
		geometry mshp <- convex_hull(polygon(all_location));
		
		// il met a jour tout ces nouvelle cellule
		mshp <- 20#m around mshp;
		ask cells overlapping mshp
		{
	...
	reflex eat
	{
		...
		loop while:qt_vole_to_eat > 0
		{
			...
			float available_meat <- sum(first_elements collect(each.associated_cell.total_population));
			...
			ask first_elements
			{
				...
				add self to:myself.used_knowledge;
			}
		...
		// il prend les 10, si pas assez va dans les 10 suivantes etc....
		
		
		// S'il n'y a pas assez de campagnols dans les n cellules alors elles sont vidées et le complément est chercher dans les cellules de la base de connaissance
	}
...
grid cells cell_width:100#m cell_height:100#m schedules:  simulated_plots
{
	...
	list<float> kill_all(bool adult, bool juvenile, bool young)
	{
		...
		return list([res_qte_adult, res_qte_juve, res_qte_younger]);
	}
	
	// convertit des grammes en individus
	list<float> convert_gram2ind(float adult,float juve, float young) {
		float i_adult <- adult/adult_weight;
		float i_juvenile <- juve / young_weight;
	...
	list<float> kill(float g_adult,float g_juve, float g_young) {
		list<float> ml <- convert_gram2ind( g_adult, g_juve,  g_young);
		...
		return list([res_qte_adult,res_qte_juve,res_qte_young]);
 ...
 */	
	output {
		...
		{
			species death_factor aspect:base;
		}
